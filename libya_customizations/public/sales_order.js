...
frappe.ui.form.on('Sales Order Item', {
    custom_is_old: function(frm, cdt, cdn) {
        let row = locals[cdt][cdn];
        if (!row.item_code || !frm.doc.selling_price_list) {
            frappe.msgprint(__('Item code or price list missing.'));
            return;
        }
        ...
        frappe.call({
            ...
            args: {
                ...
                fields: ['custom_old_rate', 'price_list_rate'],
                limit_page_length: 1
            },
            callback: function(r) {
                if (r.message && r.message.length > 0) {
                    let price_data = r.message[0];
                    let target_rate = row.custom_is_old ? price_data.custom_old_rate : price_data.price_list_rate;

                    if (target_rate != null) {
                        frappe.model.set_value(cdt, cdn, 'rate', target_rate);
                    } else {
                        frappe.msgprint(__('The rate could not be found for this item and price list.'));
                    }
                } else {
                    frappe.msgprint(__('Item Price record not found.'));
                }
            }
        });
    }
